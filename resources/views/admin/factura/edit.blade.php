            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="total" name="total"
                        value="{{ old('total', $factura->total) }}" required>
                    @error('total')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="servicios[]" class="form-select" id="servicios" multiple required>
                        @foreach ($servicios as $servicio)
                            <option value="{{ $servicio->id }}" @selected(in_array($servicio->id, old('servicios', $factura->detalle->pluck('id_servicio')->toArray() ?? [])))>
                                {{ $servicio->nombre }} - ${{ $servicio->precio }}
                            </option>
                        @endforeach
                    </select>
                    <small class="text-muted">Mantenga presionada la tecla Ctrl para seleccionar múltiples
                        servicios.</small>
                    @error('servicios')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
